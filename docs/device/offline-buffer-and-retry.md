...
1. 先產生 `LOCAL_BUFFER_NEAR_CAPACITY`。
2. 降低非必要 diagnostic/heartbeat 頻率，不降低 radar parser 優先級。
3. 到容量上限時依明確 policy 淘汰最舊且超過最大 age 的窗口，記錄安全 dropped summary；不可無限擴張或覆蓋未記錄。
4. 恢復連線後加入 `OFFLINE_GAP`、`NETWORK_UPLOAD_DELAYED`，讓後端降低 freshness/confidence。

Flash 採批次寫入、頁面輪替與寫入次數監控方向，避免每 frame 寫入。正式容量、wear leveling、掉電一致性與 NVS/partition 選擇需在 Phase 8.5/9 以硬體測試定案。

## 時鐘未同步

Clock-unsynced window 保留 monotonic uptime、boot/sequence、duration 與 `CLOCK_UNSYNCED`。後端可用 `receivedAt` 和相鄰已同步點估算排序，但不得宣稱其 absolute occurred time 精確。
